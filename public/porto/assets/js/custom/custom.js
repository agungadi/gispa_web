...
if (typeof landing == 'undefined') $('input:text:not([readonly="readonly"])').first().focus();
...
window.isValidFile = function (inputSelector, validationMessageSelector) {
  ...
  if ($.inArray(ext, ['svg']) == -1) {
    ...
    $(validationMessageSelector).html('The image must be a file of type: svg.').show();
    return false;
  }
...
window.isValidImg = function (inputSelector, validationMessageSelector) {
  ...
  if ($.inArray(ext, ['png', 'jpg', 'jpeg', 'svg']) == -1) {
    $(inputSelector).val('');
    $(validationMessageSelector).removeClass('d-none');
    $(validationMessageSelector).html('The image must be a file of type: png ,jpg ,jpeg ,svg.').show();
    setTimeout(function () {
      $('#validationErrorsBox').delay(5000).slideUp(300);
    });
    return false;
...
$(document).ready(function () {
  ...
  var hasActiveMenu = $(document).find('.nav-item .collapse ul li').hasClass('active');

  if (hasActiveMenu) {
    $(document).find('.nav-item .collapse ul li.active').parent('ul').parent('ul').addClass('show');
